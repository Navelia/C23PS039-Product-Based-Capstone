...
async function getData(kategori) {
    let url = `https://${kategori.toLowerCase()}.kompas.com`;
    ...
    try {
        let response = await axios.get(url).catch((error) => {
            ...
            if (error.code == "ENOTFOUND") {
                url = `https://www.kompas.com/${kategori.toLowerCase()}`;
                tryagain = true;
            }
...
function parse1($, e) {
    ...
    if (title != "" && image_thumbnail != undefined) {
        ...
        var image_full = image_thumbnail
        .replace(/crops.*data/, 'data')
        .replace("crops", "");

        var time = $('.article__date', e)
        .text()
        .trim()
        .replace(" WIB", "");
...
function parse3($, e) {
    ...
    const title = $('.trenLatest__box h3.trenLatest__title a', e)
    .text()
    .replace("\n", "")
    .trim();
...
async function getDetail(kategori, slug) {
    let url = `https://${kategori.toLowerCase()}.kompas.com/${slug}?page=all`;

    if (kategori.toLowerCase() == "baca") {
        url = `https://kompas.com/${kategori.toLowerCase()}/${slug}?page=all`;
    }
    ...
    try {
        ...
        const image = $('.cover-photo img', _element_article)
        .attr('src')
        .replace(/crops.*data/, 'data')
        .replace("crops", "");
